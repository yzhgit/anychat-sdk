...
void FileManagerImpl::upload(
    ...
    if (slash != std::string::npos)
        file_name = local_path.substr(slash + 1);
#ifdef _WIN32
    ...
    nlohmann::json req_body = { { "fileName", file_name },
                                { "fileType", file_type },
                                { "fileSize", 0 },
    ...
    http_->post(
        ...
        [this, local_path, on_progress, on_done](network::HttpResponse resp) {
            if (!resp.error.empty()) {
                if (on_done)
                    on_done(false, FileInfo{}, resp.error);
                return;
            }
            if (resp.status_code < 200 || resp.status_code >= 300) {
                if (on_done)
                    on_done(false, FileInfo{}, "upload-token failed: " + resp.body);
                return;
            }
            ...
            try {
                ...
                if (on_done)
                    on_done(false, FileInfo{}, std::string("parse error: ") + e.what());
                return;
            }
            ...
            if (!ifs.is_open()) {
                if (on_done)
                    on_done(false, FileInfo{}, "cannot open file: " + local_path);
                return;
            }
            std::string file_bytes((std::istreambuf_iterator<char>(ifs)), std::istreambuf_iterator<char>());
            ifs.close();
            ...
            http_->put(
                ...
                [this, file_id, file_bytes_size = file_bytes.size(), on_progress, on_done](
                    ...
                    if (put_resp.status_code < 200 || put_resp.status_code >= 300) {
                        if (on_done)
                            on_done(false, FileInfo{}, "PUT failed: " + put_resp.body);
                        return;
                    }

                    // Notify progress: upload complete
                    if (on_progress) {
                        on_progress(static_cast<int64_t>(file_bytes_size), static_cast<int64_t>(file_bytes_size));
                    }

                    // Step 3: POST /files/{file_id}/complete to activate the file
                    std::string complete_path = "/files/" + file_id + "/complete";
                    http_->post(complete_path, "{}", [on_done](network::HttpResponse complete_resp) {
                        if (!complete_resp.error.empty()) {
                            if (on_done)
                                on_done(false, FileInfo{}, complete_resp.error);
                            return;
                        }
                        if (complete_resp.status_code < 200 || complete_resp.status_code >= 300) {
                            if (on_done)
                                on_done(false, FileInfo{}, "complete failed: " + complete_resp.body);
                            return;
                        }
...
void FileManagerImpl::getDownloadUrl(
    ...
    http_->get(path, [cb](network::HttpResponse resp) {
        ...
        if (resp.status_code < 200 || resp.status_code >= 300) {
            if (cb)
                cb(false, "", "getDownloadUrl failed: " + resp.body);
            return;
        }
...
void FileManagerImpl::deleteFile(const std::string& file_id, FileCallback cb) {
    ...
    http_->del(path, [cb](network::HttpResponse resp) {
        ...
        if (resp.status_code < 200 || resp.status_code >= 300) {
            if (cb)
                cb(false, "deleteFile failed: " + resp.body);
            return;
        }
        if (cb)
            cb(true, "");
