...
void FriendManagerImpl::getPendingRequests(FriendRequestListCallback cb) {
    http_->get("/friends/requests?type=received", [cb = std::move(cb)](network::HttpResponse resp) {
        if (!resp.error.empty()) {
            cb({}, resp.error);
            return;
        }
...
void FriendManagerImpl::handleFriendNotification(const NotificationEvent& event) {
    ...
    if (type != "friend.request" && type != "friend.request_handled" && type != "friend.deleted"
        && type != "friend.remark_updated" && type != "friend.blacklist_changed") {
        return;
    }
